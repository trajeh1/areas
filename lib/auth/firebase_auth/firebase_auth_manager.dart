      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            'pqr3ezhh' /* مر وقت طويل من آخر تسجيل دخول.... */,
          ))),
        );
      }
    }
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(FFLocalizations.of(context).getText(
            'fjuvpbtc' /* مر وقت طويل من آخر تسجيل دخول،... */,
          ))),
        );
      }
    }
  ...
  Future resetPassword({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(FFLocalizations.of(context).getText(
          '63mndve2' /* حدث خطأ أثناء محاولة تسجيل الد... */,
        ))),
      );
      return null;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text(FFLocalizations.of(context).getText(
        'q3r39pcq' /* يرجى متابعة بريدكم الالكتروني ... */,
      ))),
    );
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : ProtectedAreaFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' => FFLocalizations.of(context).getText(
            '4lyhj1mq' /* البريد الالكتروني المدخل مستخد... */,
          ),
        'INVALID_LOGIN_CREDENTIALS' => FFLocalizations.of(context).getText(
            'x3dx2wxo' /* اسم المستخدم أو كلمة المرور غي... */,
          ),
